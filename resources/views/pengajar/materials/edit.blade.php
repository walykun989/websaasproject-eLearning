...
@section('title', 'Edit Materi - Pengajar')
...
<div class="py-6 bg-white">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="md:flex md:items-center md:justify-between mb-6">
            <div>
                <h2 class="text-2xl font-light text-black">Edit Materi</h2>
                <p class="mt-1 text-sm font-light text-gray-600">{{ $course->title }}</p>
            </div>
        </div>

        <div class="bg-white border border-gray-200 p-6">
            <form method="POST" action="{{ route('pengajar.courses.materials.update', [$course, $material]) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $material->title) }}" required class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white text-black font-light focus:outline-none focus:border-black @error('title') border-red-500 @enderror">
                        @error('title')
                    ...
                    <div>
                        ...
                        <select name="content_type" id="content_type" required class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white text-black font-light focus:outline-none focus:border-black @error('content_type') border-red-500 @enderror">
                            ...
                            <option value="video" {{ old('content_type', $material->content_type) == 'video' ? 'selected' : '' }}>Video</option>
                            <option value="text" {{ old('content_type', $material->content_type) == 'text' ? 'selected' : '' }}>Text</option>
                        </select>
                        @error('content_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="5" required class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white text-black font-light focus:outline-none focus:border-black @error('content') border-red-500 @enderror">{{ old('content', $material->content) }}</textarea>
                        @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="tier_required" id="tier_required" required class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white text-black font-light focus:outline-none focus:border-black @error('tier_required') border-red-500 @enderror">
                            ...
                            <option value="free" {{ old('tier_required', $material->tier_required) == 'free' ? 'selected' : '' }}>Free</option>
                            <option value="apik" {{ old('tier_required', $material->tier_required) == 'apik' ? 'selected' : '' }}>Apik</option>
                            <option value="sangar" {{ old('tier_required', $material->tier_required) == 'sangar' ? 'selected' : '' }}>Sangar</option>
                        </select>
                        @error('tier_required')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', $material->duration_minutes) }}" min="1" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white text-black font-light focus:outline-none focus:border-black @error('duration_minutes') border-red-500 @enderror">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $material->is_published) ? 'checked' : '' }} class="h-4 w-4 border-gray-300 bg-white focus:ring-0 focus:ring-offset-0">
