...
<div class="py-6 bg-white min-h-screen transition-colors duration-200">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-cyan-50 border-2 border-black p-8 mb-6 transition-colors duration-200">
            <h3 class="text-2xl font-light text-black mb-6 border-b-2 border-black pb-3">Profil Pengajar</h3>
            ...
            <div class="flex flex-col md:flex-row items-center gap-8">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm font-light text-gray-600 mb-4">
                        Foto profil Anda akan ditampilkan kepada peserta. Format: JPG, PNG, GIF (maks 2MB)
                    </p>
                    <div class="space-y-2 text-sm font-light text-gray-600">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="w-2 h-2 bg-black rounded-full"></span>
                            <span>Foto akan ditampilkan di halaman kelas Anda</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-cyan-50 border-2 border-black transition-colors duration-200">
            <div class="px-4 py-5 sm:p-6">
                ...
                <form method="POST" action="{{ route('pengajar.profile.update') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        <label for="bio" class="block text-sm font-light text-black">Bio / Deskripsi</label>
                        <textarea name="bio" id="bio" rows="4"
                            class="mt-1 block w-full border-2 border-black bg-white text-black font-light py-2 px-3 focus:outline-none focus:ring-2 focus:ring-black"
                            placeholder="Ceritakan sedikit tentang diri Anda dan keahlian Anda...">{{ old('bio', auth()->user()->bio) }}</textarea>
                        <p class="mt-2 text-xs font-light text-gray-600">
                            Maks 1000 karakter. Ditampilkan di profil pengajar Anda.
                        </p>
                    </div>

                    <div>
                        <label for="instagram_link" class="block text-sm font-light text-black">Link Instagram</label>
                        <input type="url" name="instagram_link" id="instagram_link" value="{{ old('instagram_link', auth()->user()->instagram_link) }}"
                            class="mt-1 block w-full border-2 border-black bg-white text-black font-light py-2 px-3 focus:outline-none focus:ring-2 focus:ring-black"
                            placeholder="https://instagram.com/username">
                        <p class="mt-2 text-xs font-light text-gray-600">
                            Link profil Instagram Anda akan ditampilkan di halaman kelas.
                        </p>
                    </div>
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('pengajar.dashboard') }}" class="inline-flex items-center px-4 py-2 border-2 border-black text-sm font-light text-black bg-white hover:bg-gray-50 transition-colors">
                            Batal
                        </a>
