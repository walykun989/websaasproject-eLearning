...
<div class="py-12 bg-white">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-cyan-50 border-2 border-black">
            <div class="px-6 py-8">
                <h3 class="text-lg leading-6 font-light text-black">Tambah Kursus Baru</h3>

                <form method="POST" action="{{ route('admin.courses.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="pengajar_id" id="pengajar_id" required class="mt-1 block w-full border-2 border-black px-3 py-2 bg-white text-black font-light focus:outline-none focus:border-black">
                            ...
                            <option value="{{ $pengajar->id }}" {{ old('pengajar_id') == $pengajar->id ? 'selected' : '' }}>{{ $pengajar->name }}</option>
                            @endforeach
                        </select>
                        @error('pengajar_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        <label for="thumbnail" class="block text-sm font-light text-black">Thumbnail</label>
