...
@section('title', 'Laporan Penjualan Tahunan - Admin')
...
<div class="py-6 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="md:flex md:items-center md:justify-between mb-6">
            <h2 class="text-2xl font-light text-black">Yearly Sales Report</h2>
        ...
        <div class="mb-6 bg-white border-2 border-black p-4">
            <form method="GET" action="{{ route('admin.reports.yearly') }}" class="flex items-end gap-4">
                <div class="flex-1">
                    <label for="year" class="block text-sm font-light text-black mb-2">Select Year</label>
        ...
        <div class="mb-6 bg-white border-2 border-black p-6">
            <h3 class="text-lg font-light text-black mb-4">Total Revenue for {{ $year }}</h3>
        ...
        <!-- Sales by Month -->
        <div class="mb-6 bg-white border-2 border-black">
            <div class="p-6">
                <h3 class="text-lg font-light text-black mb-4">Revenue by Month</h3>
                @if($salesByMonth->isNotEmpty())
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @foreach($salesByMonth as $month => $revenue)
                    <div class="border border-gray-200 p-4">
                        <dt class="text-sm font-light text-gray-600">{{ $month }}</dt>
                        <dd class="mt-1 text-lg font-light text-black">Rp {{ number_format($revenue, 0, ',', '.') }}</dd>
                    </div>
                    @endforeach
                </div>
                @else
                <p class="text-center py-4 text-sm font-light text-gray-600">No sales data for this year</p>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white border-2 border-black">
            <div class="p-6">
                <h3 class="text-lg font-light text-black mb-4">All Transactions</h3>
                @if($sales->isNotEmpty())
                ...
                @else
                <p class="text-center py-8 text-sm font-light text-gray-600">No sales for this year</p>
                @endif
            </div>
        </div>
    </div>
