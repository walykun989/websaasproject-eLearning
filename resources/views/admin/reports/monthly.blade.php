...
@section('title', 'Laporan Penjualan Bulanan - Admin')
...
<div class="py-6 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="md:flex md:items-center md:justify-between mb-6">
            <h2 class="text-2xl font-light text-black">Monthly Sales Report</h2>
        ...
        <div class="mb-6 bg-white border-2 border-black p-4">
            <form method="GET" action="{{ route('admin.reports.monthly') }}" class="flex items-end gap-4">
                <div class="flex-1">
                    <label for="month" class="block text-sm font-light text-black mb-2">Month</label>
                    <select name="month" id="month" class="block w-full border-2 border-black px-3 py-2 bg-white text-black font-light focus:outline-none focus:border-black">
                        @for($m = 1; $m <= 12; $m++)
                ...
                <div class="flex-1">
                    <label for="year" class="block text-sm font-light text-black mb-2">Year</label>
                    <select name="year" id="year" class="block w-full border-2 border-black px-3 py-2 bg-white text-black font-light focus:outline-none focus:border-black">
        ...
        <div class="mb-6 bg-white border-2 border-black p-6">
            <h3 class="text-lg font-light text-black mb-4">Total Revenue for {{ \Carbon\Carbon::create()->month((int)$month)->format('F') }} {{ $year }}</h3>
        ...
        <div class="mb-6 grid grid-cols-1 gap-5 sm:grid-cols-3">
            <div class="bg-white border-2 border-black p-5">
                <dt class="text-sm font-light text-gray-600">Free Tier Sales</dt>
                <dd class="mt-1 text-2xl font-light text-black">Rp {{ number_format($salesByTier->get('free', 0), 0, ',', '.') }}</dd>
            </div>
            <div class="bg-white border-2 border-black p-5">
                <dt class="text-sm font-light text-gray-600">Apik Tier Sales</dt>
                <dd class="mt-1 text-2xl font-light text-black">Rp {{ number_format($salesByTier->get('apik', 0), 0, ',', '.') }}</dd>
            </div>
            <div class="bg-white border-2 border-black p-5">
                <dt class="text-sm font-light text-gray-600">Sangar Tier Sales</dt>
                <dd class="mt-1 text-2xl font-light text-black">Rp {{ number_format($salesByTier->get('sangar', 0), 0, ',', '.') }}</dd>
            </div>
        </div>
        ...
        <div class="bg-white border-2 border-black">
            <div class="p-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-light text-gray-600">{{ $order->verified_at->format('d M Y') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                @else
                <p class="text-center py-8 text-sm font-light text-gray-600">No sales for this month</p>
                @endif
            </div>
        </div>
    </div>
